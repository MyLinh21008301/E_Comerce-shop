...
export default function Dashboard() {
  ...
  const navigationItems = [
    { icon: "home", label: "Trang chủ", href: "/dashboard", isActive: true },
          ...
          <div>
            {navigationItems.map((item, index) => (
              <Link
                key={index}
                href={item.href}
                className={`flex items-center gap-3 px-6 py-3 text-gray-700 hover:bg-gray-50 hover:text-[#ee4d2d] transition-colors ${
                  item.isActive ? 'text-[#ee4d2d] bg-gray-50' : ''
                }`}
              >
